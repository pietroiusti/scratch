/*
  ...
  - 1 prints keyboard events (the device to be listened to must be passed as first arg).
  - 2 prints the name of the x window when we change focus.

  Compile with:
  $ gcc 02_print_active_window_with_xlib_and_events_with_libevdev.c `pkg-config --cflags libevdev` `pkg-config --libs libevdev x11` -pthread -o name_of_executable
*/
...
main(int argc, char **argv)
{
    ...
    thread_return_value = pthread_create(&xthread, NULL, xfun, NULL);
